...
type Weight = "normal" | "bold" | "400" | "700";

const colors = Object.keys(theme.colors);
const sizes = ["small", "medium", "large", "extra-large"];
const weights = ["normal", "bold", "400", "700"];
...
colors.map((color) =>
  sizes.map((size) =>
    weights.map((weight) => matriz.push([size, weight, color]))
  )
);
...
describe("Test case for Label component", () => {
  ...
  it.each(colors)(
    "should be able to render the Label component for %s color and match with the snapshot",
    (color) => {
